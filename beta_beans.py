...
def menu():
    ...
    btn_add_new = tk.Button(window, text="Add a new bean", command=lambda: on_selection("Add a new bean"),
                            bg=btn_color, fg="white")
    btn_add_new.pack(pady=5)

    btn_see_all = tk.Button(window, text="See all beans", command=lambda: on_selection("See all beans"),
                            bg=btn_color, fg="white")
    btn_see_all.pack(pady=5)

    btn_find_bean = tk.Button(window, text="Find a bean by name",
                              command=lambda: on_selection("Find a bean by name"), bg=btn_color, fg="white")
    btn_find_bean.pack(pady=5)

    btn_best_method = tk.Button(window, text="See the best preparation method",
                                command=lambda: on_selection("See the best preparation method"), bg=btn_color,
                                fg="white")
    btn_best_method.pack(pady=5)

    btn_delete_bean = tk.Button(window, text="Delete bean", command=lambda: on_selection("Delete bean"),
                                bg=btn_color, fg="white")
    btn_delete_bean.pack(pady=5)

    btn_edit_bean = tk.Button(window, text="Edit bean", command=lambda: on_selection("Edit bean"), bg=btn_color,
                              fg="white")
...
def login(window, entry_username, entry_password):
...
def register(window, entry_username, entry_password):
    username = entry_username.get()
    password = entry_password.get()
    ...
    entry_username.delete(0, tk.END) # Очистка поля введення для імені користувача
    entry_password.delete(0, tk.END) # Очистка поля введення для пароля
...
if __name__ == "__main__":
    ...
    btn_login = tk.Button(window, text="Login", command=lambda: login(window, entry_username, entry_password),
                          bg=btn_color, fg="white")
    btn_login.pack()

    btn_register = tk.Button(window, text="Register", command=lambda: register(window, entry_username, entry_password),
                             bg=btn_color, fg="white")
    btn_register.pack()
